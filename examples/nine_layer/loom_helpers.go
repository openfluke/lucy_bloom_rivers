...
func marshalNet(dt poly.DType, layer map[string]any) ([]byte, error) {
	...
	spec := map[string]any{
		"id": "nine-layer-bridge", "depth": 1, "rows": 1, "cols": 1, "layers_per_cell": 1,
		"layers": []any{layer},
	}
...
func tensorToBytes(in any, dtypeLabel string) ([]byte, error) {
	switch dtypeLabel {
	case "INT8":
		// OpenVINO INT8 path uses FP32 tensors + dynamic quant on NPU (see bench.cpp).
...
// RunMultiHopDemo: Loom Dense → Intel MatMul (NPU) → Loom ReLU — compile once, many forwards.
func RunMultiHopDemo() {
	fmt.Println("\n--- Multi-hop: Loom Dense → Intel MatMul → Loom ReLU (medium, FP32) ---")
	...
	if err != nil {
		...
		fmt.Println("NPU unavailable — using CABI CPU for middle hop")
	} else {
		fmt.Println("Middle hop on NPU")
	}
	defer plug.Close()
	...
	fmt.Printf("Steady per-hop (median of %d): Loom-pre=%.3f ms  copy=%.3f ms  Intel=%.3f ms  Loom-post=%.3f ms\n",
