...
// RunDispatchSuite exercises DispatchLayer → Intel CABI (init once, infer many).
func RunDispatchSuite(sizes []string) {
	...
	reg, err := poly.DiscoverAccel(accel.AccelConfig{IntelSO: DefaultPluginPath()})
	if err != nil {
		fmt.Println("DiscoverAccel:", err)
		return
	}
	defer reg.Close()

	hasNPU := reg.IntelNPU != nil
...
func runDispatchCase(
	...
	// Intel CPU via DispatchLayer
	l.ExecTarget = accel.ExecIntelCPU
	...
	if hasNPU {
		l.ExecTarget = accel.ExecIntelNPU
		l.AccelBinding = nil
		if err := net.SyncToAccel(sizeName); err != nil {
...
func printDispatchTable(rows []dispatchRow, hasNPU bool) {
	...
	fmt.Println()
	fmt.Println("=== DispatchLayer — Loom CPU vs Intel CPU/NPU (seven-style tables) ===")
