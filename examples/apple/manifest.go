...
// BenchManifest mirrors accel/apple/bench_manifest.json.
...
func manifestPath() string {
	if v := os.Getenv("LOOM_APPLE_MANIFEST"); v != "" {
		return v
	}
	if root := os.Getenv("LOOM_ROOT"); root != "" {
		p := filepath.Join(root, "accel", "apple", "bench_manifest.json")
		if fileExists(p) {
			return p
		}
	}
	...
	return filepath.Join("accel", "apple", "bench_manifest.json")
}
...
func findManifestUpward() string {
	...
	name := filepath.Join("accel", "apple", "bench_manifest.json")
