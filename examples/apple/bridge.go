...
// RunBridgeSuite exercises Loom → Apple CABI handover for all manifest layers/dtypes.
...
func RunBridgeSuite(sizes []string, onlyLayer ...string) {
	...
	if err != nil {
		fmt.Println("Apple CABI CPU:", err)
		return
	}
	defer cpuPlug.Close()

	var gpuPlug accel.Plugin
	if GPUReady(path) {
		gpuPlug, err = openPlugin("GPU")
		if err != nil {
			fmt.Printf("GPU plugin: %v (CPU-only run)\n", err)
		} else {
			defer gpuPlug.Close()
		}
	}
	...
	for _, sizeName := range sizeOrder {
		...
		for _, layer := range m.Layers {
			...
			for _, dtypeLabel := range m.DTypes {
				...
				if gpuPlug != nil {
					gpuRow := base
					gpuRow.Device = "CABI-GPU"
					fillCABI(&gpuRow, gpuPlug, desc, weights, inBuf, dtypeLabel, warmup, iters)
