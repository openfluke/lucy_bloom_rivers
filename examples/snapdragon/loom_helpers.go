...
func marshalNet(dt poly.DType, layer map[string]any) ([]byte, error) {
	...
	spec := map[string]any{
		"id": "snapdragon-bridge", "depth": 1, "rows": 1, "cols": 1, "layers_per_cell": 1,
		"layers": []any{layer},
	}
...
func tensorToBytes(in any, dtypeLabel string) ([]byte, error) {
	switch dtypeLabel {
	case "INT8":
		// QNN HTP INT8 path: host uploads dequantized FP32; the backend requantizes
		// using the per-tensor scale/offset chosen in the plugin.
...
// RunMultiHopDemo: Loom Dense → Qualcomm MatMul (HTP) → Loom ReLU — compile once, many forwards.
func RunMultiHopDemo() {
	fmt.Println("\n--- Multi-hop: Loom Dense → Qualcomm MatMul → Loom ReLU (medium, FP32) ---")
	...
	if err != nil {
		...
		fmt.Println("NPU unavailable — using QnnCpu reference backend for middle hop")
	} else {
		fmt.Println("Middle hop on Hexagon HTP")
	}
	defer plug.Close()
	...
	fmt.Printf("Steady per-hop (median of %d): Loom-pre=%.3f ms  copy=%.3f ms  Qualcomm=%.3f ms  Loom-post=%.3f ms\n",
