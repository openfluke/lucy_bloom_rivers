...
// RunBridgeSuite exercises Loom → Qualcomm CABI handover for all manifest layers/dtypes.
...
func RunBridgeSuite(sizes []string, onlyLayer ...string) {
	...
	if err := accel.PrepareQualcommRuntime(); err != nil {
		fmt.Println("QNN runtime:", err)
		fmt.Println("Try: . accel/qualcomm/setup_env.ps1  (from Loom repo root)")
		return
	}
	cpuPlug, err := openPlugin("CPU")
	if err != nil {
		fmt.Println("Qualcomm CABI CPU:", err)
		return
	}
	defer cpuPlug.Close()
